...
function isPlayerXWinner (enemyBoat: Sprite[][], hitOrMissPX: Sprite[]) {
    killCount = 0
    for (let index = 0; index <= 2; index++) {
        currentBoatBoomCounter = 0
        for (let currentBoatSprite of enemyBoat[index]) {
        ...
        if (currentBoatBoomCounter == enemyBoat[index].length) {
            killCount += 1
        }
    }
    return killCount
